...
public class Frostburn extends Buff implements Hero.Doom {
	...
	public boolean act() {
		...
		if (Dungeon.level.flamable[target.pos] && Blob.volumeAt(target.pos, FrostFire.class) == 0) {
			GameScene.add( Blob.seed( target.pos, 3, FrostFire.class ) );
		}
		...
		if (left <= 0 ||
			...
			if (Dungeon.level.water[target.pos] && !target.flying) {
				Buff.prolong(target, Frost.class, Frost.DURATION);
				CellEmitter.get(target.pos).start(SnowParticle.FACTORY, 0.1f, 12);
			}
		}
	...
	public float speedFactor(){
		return Math.max(0.5f, 1 - left*0.04f);
	}
	...
	@Override
	public String desc() {
		return Messages.get(this, "desc", dispTurns(left), Messages.decimalFormat("#.##", (1f-speedFactor())*100f));
	}
