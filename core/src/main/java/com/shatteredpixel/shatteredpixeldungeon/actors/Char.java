...
public abstract class Char extends Actor {
	...
	public boolean interact(Char c){
		...
		if (paralysed > 0 || c.paralysed > 0 || rooted || c.rooted
				|| buff(VertigoLike.class) != null || c.buff(VertigoLike.class) != null){
			return true;
		}
	...
	public int defenseProc( Char enemy, int damage ) {
		...
		if (buff(NecklaceOfIce.necklaceRecharge.class) != null &&
			buff(Chill.class) != null){
			damage -= Math.min(damage, Chill.necklaceBlocking());
		}
	...
	public void damage( int dmg, Object src ) {
		...
		if (sprite != null) {
			...
			if (src instanceof Chill || src instanceof Frost || src instanceof Frostburn)           icon = FloatingText.FROST;
	...
	public float resist( Class effect ){
		...
		if (Dungeon.alchemist && areRelated(effect, Wand.class))
			result *= 0;
